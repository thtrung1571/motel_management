...
const RoomList = () => {
  ...
  // Kiểm tra ca làm việc khi component mount
  useEffect(() => {
    checkCurrentShift();
  }, []);
  ...
  const checkCurrentShift = async () => {
    try {
      ...
      if (response.data.status === 'success' && response.data.data) {
        // Kiểm tra nếu có ca làm việc và status là 'active'
  ...
  const handleStartShift = async () => {
    try {
      ...
      toast.error('Không thể bắt đầu ca làm việc');
    }
  };
  ...
  return (
    <Box sx={{ pt: 0.5 }}>
      ...
      >
        ...
        <Button
          variant="contained"
          color={currentShift?.status === 'active' ? "warning" : "primary"}
          startIcon={currentShift?.status === 'active' ? <ExitToApp /> : <PlayArrow />}
          onClick={currentShift?.status === 'active' ? handleEndShift : handleStartShift}
          sx={{ 
            borderRadius: 2,
            textTransform: 'none',
            boxShadow: 2
        ...
        >
          {currentShift?.status === 'active' ? 'Giao Ca' : 'Nhận Ca'}
        </Button>
      </Stack>
