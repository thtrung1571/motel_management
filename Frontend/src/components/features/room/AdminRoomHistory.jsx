...
const AdminRoomHistory = () => {
  ...
  const RentalDetailModal = ({ rental, onClose, isAdmin }) => {
            ...
            {isAdmin && (
              <Grid item xs={12}>
                <Paper elevation={0} variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
                  ...
                  <Stack spacing={1}>
                    <Box display="flex" justifyContent="space-between">
                      <Typography color="text.secondary">Tạo lúc:</Typography>
                      <Typography>{formatDateTime(rental.createdAt)}</Typography>
                    </Box>
                    <Box display="flex" justifyContent="space-between">
                      <Typography color="text.secondary">Cập nhật lúc:</Typography>
                      <Typography>{formatDateTime(rental.updatedAt)}</Typography>
                    </Box>
                    {rental.hasWarning && (
                      <Box display="flex" justifyContent="space-between">
                        <Typography color="text.secondary">Cảnh báo:</Typography>
                        <Typography color="error">{rental.warningDetails}</Typography>
                      </Box>
                    )}
                  </Stack>
                </Paper>
  ...
  const getTimeDifferenceStatus = (time1, time2) => {
    if (!time1 || !time2) return null;
    
    const diffInMinutes = Math.abs(
      (new Date(time1) - new Date(time2)) / (1000 * 60)
    );
    
    if (diffInMinutes <= 10) return { color: 'success.main', icon: CheckCircleIcon };
    if (diffInMinutes <= 30) return { color: 'warning.main', icon: WarningIcon };
    return { color: 'error.main', icon: ErrorIcon };
  };

  const TimeDisplay = ({ label, time, compareTime }) => {
    const status = compareTime ? getTimeDifferenceStatus(time, compareTime) : null;
    ...
    return (
      <TableCell>
        ...
        >
          {formatDateTime(time)}
          {status && Icon && (
            <Tooltip title={`Chênh lệch thời gian ${label}`}>
              <Icon 
                fontSize="small" 
            ...
            {loading ? (
              ...
              history.map((rental) => (
                <TableRow key={rental.id}>
                  ...
                  <TimeDisplay 
                    label="check-in"
                    time={rental.checkInTime} 
                    compareTime={rental.createdAt}
                  />
                  <TimeDisplay 
                    label="check-out"
                    time={rental.checkOutTime}
                    compareTime={rental.updatedAt}
                  />
                  ...
                  <TableCell>
                    {rental.updatedAt ? formatDateTime(rental.updatedAt) : '-'}
                  </TableCell>
                  <TableCell>
                    <Button
                      variant="outlined"
...
export default AdminRoomHistory; 